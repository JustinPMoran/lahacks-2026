...
- **Mongo → local JSONL motion bus**: The Pi live UI path runs both `backend/motion_plot.py` (producer, reads receiver C6 over serial) and `frontend/main.py` (consumer, `MotionDataPoller`). The previous UI round-trip through MongoDB Atlas was wasted — replaced with a local append-only JSONL file at `/tmp/ruview_motion.jsonl` (override via `RUVIEW_MOTION_LOG`). `motion_plot.py` now uses `FileSink` (truncate-on-startup, line-buffered append). The frontend poller tails the file (offset-tracked, handles truncation/rotation, prunes entries older than `history_seconds`). `backend/mongo_logger.py` remains as a separate Atlas/Gemini training-data ingestion utility.
...
- **Cleanup (April 25 evening)**: removed unused experimental scripts from repo root: `motion_detect.py`, `presence_detect.py`, `collect_labels.py`, `train_count.py`, `demo_pi.py`, `mongo_logger.py`, plus `baseline.npz`. None referenced by `backend/` or `frontend/`. requirements.txt already lean (no sklearn/pandas).
...
## April 25, 2026 Addendum (Merge Resolution)
- Resolved the `origin/main` merge by keeping the local Atlas/Gemini training ingestion path (`backend/mongo_logger.py`, `.env.example`, `pymongo`, `google-genai`) alongside the remote local-JSONL UI path.
- Combined the expanded-map target-lock UI with proximity markers in `frontend/main.py` so both remote and local map behavior remain available.
- Rationale: the live Raspberry Pi dashboard should stay low-latency through JSONL, while training capture runs as an explicit Atlas logger when needed.
