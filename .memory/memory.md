...
- **Mongo → local JSONL motion bus**: The Pi runs both `backend/motion_plot.py` (producer, reads receiver C6 over serial) and `frontend/main.py` (consumer, `MotionDataPoller`). The previous round-trip through MongoDB Atlas was wasted — replaced with a local append-only JSONL file at `/tmp/ruview_motion.jsonl` (override via `RUVIEW_MOTION_LOG`). `motion_plot.py` now uses `FileSink` (truncate-on-startup, line-buffered append). The frontend poller tails the file (offset-tracked, handles truncation/rotation, prunes entries older than `history_seconds`). Removed `pymongo` from `requirements.txt`. Deleted unused Mongo-only scripts `backend/mongo_logger.py` and `backend/seed_demo_data.py`. The `MONGODB_*` env vars in `.env` are now dead and can be removed.
